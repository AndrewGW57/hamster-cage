...
async function callVision<T>(imageBase64: string, prompt: string): Promise<T> {
...
export async function parseCTP(imageBase64: string): Promise<ParsedCTPEntry[]> {
...
Return ONLY a JSON array: { "name": string, "position": number, "hole": number, "distance": string }`
  ...
  return callVision<ParsedCTPEntry[]>(imageBase64, prompt)
}

export async function parseLD(imageBase64: string): Promise<ParsedLDEntry[]> {
...
Return ONLY a JSON array: { "name": string, "position": number, "hole": number, "distance_yards": string }`
  ...
  return callVision<ParsedLDEntry[]>(imageBase64, prompt)
}
